...
def Fmeasure_calu(pred, gt, threshold):
    ...
    IoU = NumAnd / (FN + NumRec + epsilon)
    PreFtem = NumAnd / (NumRec + epsilon)
    RecallFtem = NumAnd / (num_obj + epsilon)
    SpecifTem = TN / (TN + FP + epsilon)
    Dice = 2 * NumAnd / (num_obj + num_pred + epsilon)
    FmeasureF = (2.0 * PreFtem * RecallFtem) / (PreFtem + RecallFtem + epsilon)
    ...
    # return precision, recall, specificity, dice, fmeasure, iou
